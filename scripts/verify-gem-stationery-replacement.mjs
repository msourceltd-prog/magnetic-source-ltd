...
const stationeryProducts = products.filter((product) => product.category === "stationery-party");
...
const bestSellerCount = stationeryProducts.filter((product) => product.tags?.includes("Best seller")).length;

const valid = stationeryProducts.length === 100 && bestSellerCount === 5 && stationeryProducts.every((product) => (
...
if (!valid) throw new Error(`Gem Stationery verification failed: ${JSON.stringify({ stationery_count: stationeryProducts.length, bestSellerCount, sample: stationeryProducts.slice(0, 3) })}`);

console.log(JSON.stringify({
  total_products: products.length,
  stationery_count: stationeryProducts.length,
  stationery_best_seller_count: bestSellerCount,
  all_stationery_skus_are_gem: stationeryProducts.every((product) => product.sku.startsWith("GEM-")),
  category_counts: categoryCounts,
  sample: stationeryProducts.slice(0, 5),
}, null, 2));
