...
const query = "update public.products set image = '/product-image-pending.svg' where image = 'supplier-image-pending';";
...
if (!response.ok) throw new Error(`Placeholder normalization failed (${response.status}): ${await response.text()}`);
console.log("Normalized live supplier-image placeholders.");
