...
const html = await readFile(new URL("../data/gem-seasonal-source.html", import.meta.url), "utf8");
...
const matchText = (source, pattern, label) => {
  ...
  if (!match?.[1]) throw new Error(`Missing ${label} in a Gem Imports seasonal product card.`);
...
const products = productBlocks.map((block) => {
  ...
  return {
    ...
    slug: `gem-seasonal-${sourceSku.toLowerCase().replace(/[^a-z0-9]+/g, "-")}-${name.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "").slice(0, 60)}`,
    description: `Gem Imports seasonal wholesale line: ${name}. Supplied in packs of ${cartonQuantity}.`,
  };
});
...
if (products.length !== 100 || duplicateSkus.length) {
  throw new Error(`Expected 100 unique Gem Imports Seasonal products; found ${products.length} records and ${duplicateSkus.length} duplicates.`);
}

await writeFile(new URL("../data/gem-seasonal-import-source.json", import.meta.url), JSON.stringify({
  created_at: new Date().toISOString(),
  source: "https://www.gemimports.co.uk/dept/wholesale-seasonal-supplies_d0116.htm",
