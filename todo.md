...
- [ ] Investigate the owner-reported live Admin view that still does not show the Homepage tab; distinguish an older cached bundle from a deployment fault and restore the existing-admin hero and homepage collection controls visibly.
- [ ] Simplify the existing Admin Login into only the requested client controls: add/edit product with required category selection, three homepage hero-image controls, and Best Seller/New Arrival selection. Remove the separate category and request-management sections from the visible Admin interface without deleting product, category, or order data.
