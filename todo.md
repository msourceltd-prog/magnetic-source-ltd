...
- [x] Replace the confirmation with professional Trade Enquiry Received content, clear non-payment wording, next-step guidance, and useful return actions. New enquiry references use `MS-ENQ-`, and the receipt now confirms enquiry review, contact, availability, delivery options, and trade-price confirmation without representing payment or an accepted order.
- [x] Build and visually verify the professional confirmation page. The customer receipt now uses Trade Enquiry Received, MS-ENQ references, professional next steps, and no demo/Supabase copy; the Admin enquiry status also uses New enquiry. Production build and TypeScript validation pass, and the final customer-facing source scan finds no demo, review-flow, Supabase-connection, or MS-DEMO wording. Deployment follows the saved checkpoint.
