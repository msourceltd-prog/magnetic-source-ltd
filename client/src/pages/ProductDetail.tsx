...
export default function ProductDetail() {
  ...
  return <StoreLayout><SEOHead title={`${product.name} | Magnetic Source Ltd`} description={`${product.description} ${product.pack}, SKU ${product.sku}.`} path={`/product/${product.slug}`} image={product.image} schema={productSchema} />
    ...
    <section className="trade-shell product-detail">
      <div className="product-detail-image">{product.image === SUPPLIER_IMAGE_PLACEHOLDER ? <><div className="product-detail-placeholder" role="img" aria-label={`Supplier image pending for ${product.name}`}><span className="detail-placeholder-mark" aria-hidden="true" /></div><span className="detail-image-pending">Image pending</span></> : <img src={product.image} alt={product.name} fetchPriority="high" decoding="async" />}<span className="image-corner large" /></div>
