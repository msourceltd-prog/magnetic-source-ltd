  ...
  return <StoreLayout><SEOHead title={`${product.name} | Magnetic Source Ltd`} description={`${product.description} ${product.pack}, SKU ${product.sku}, priced in GBP.`} path={`/product/${product.slug}`} image={product.image} schema={productSchema} />
    ...
    <section className="trade-shell product-detail">
      ...
      <div className="product-detail-copy">
        ...
        <h1>{product.name}</h1>
        <p className="detail-price">{formatGBP(product.price)} <small>{product.priceBasis} · {product.pack}</small></p>
        ...
        <dl className="detail-specs"><div><dt>Brand</dt><dd>Not listed</dd></div><div><dt>Internal reference</dt><dd>{product.sku}</dd></div><div><dt>Availability</dt><dd><Check size={14} /> {product.availability}</dd></div><div><dt>Pack format</dt><dd>{product.pack}</dd></div><div><dt>Price basis</dt><dd>{product.priceBasis}</dd></div><div><dt>Dispatch</dt><dd>Trade delivery details confirmed after enquiry</dd></div></dl>
