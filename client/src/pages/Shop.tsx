...
const retiredCollectionSlugs = new Set(["best-sellers", "new-arrivals"]);
...
export default function Shop() {
  ...
  const seoDescription = activeCategoryInfo ? `Magnetic Source offers ${activeCategoryInfo.name} wholesale products for UK trade buyers, with clear product descriptions, pack details and controlled customer price access.` : "Browse Magnetic Source wholesale products for UK trade buyers, with practical retail lines, clear pack details, product references and controlled customer price access.";

  useEffect(() => {
    const requestedCategory = getQuery(searchString, "category");
    const nextCategory = retiredCollectionSlugs.has(requestedCategory) ? "" : requestedCategory;
    setActiveCategory(nextCategory);
    setSearch(getQuery(searchString, "q"));
    setSort(getQuery(searchString, "sort") || "catalogue");
    if (retiredCollectionSlugs.has(requestedCategory)) {
