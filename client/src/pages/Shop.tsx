...
const getQuery = (location: string, key: string) => new URLSearchParams(location.split("?")[1] || "").get(key) || "";

export default function Shop() {
  const [location, navigate] = useLocation();
  const initialCategory = getQuery(location, "category");
  const initialQuery = getQuery(location, "q");
  const [activeCategory, setActiveCategory] = useState(initialCategory);
  const [search, setSearch] = useState(initialQuery);
  const [stockOnly, setStockOnly] = useState(false);
  const [sort, setSort] = useState(getQuery(location, "sort") || "featured");
