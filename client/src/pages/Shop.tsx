...
export default function Shop() {
  ...
  return <StoreLayout>
    <section className="page-banner"><div className="trade-shell"><div><p className="eyebrow">Home / Shop</p><h1>{activeCategoryInfo?.name || "The trade edit"}</h1><p>{activeCategoryInfo?.summary || "A focused catalogue of practical, compact lines for independent retail and marketplace sellers."}</p></div></div></section>
    <div className="trade-shell shop-layout">
      ...
      <section className="catalogue-content">
        ...
        <div className="catalogue-meta"><span><b>Selected</b> products in this trade edit</span><span>All prices in GBP (£)</span></div>
        {filtered.length ? <><div className="catalogue-intro-strip"><span className="eyebrow">Selected for the working edit</span><p>Each product is surfaced with SKU, stock position and pack format before the basket action.</p></div><div className="product-grid catalogue-grid">{editResults.map((product, index) => <Fragment key={product.id}><ProductCard product={product} />{index === 11 && <aside className="catalogue-interrupt"><div><span className="eyebrow light">Trade pick / 01</span><h2>Compact stock.<br />Clear shelf logic.</h2><p>From stationery to home utility, the edit is deliberately paced so buyers can move by use case rather than through an endless feed.</p><Link href="/shop?tag=marketplace" className="catalogue-interrupt-link">Marketplace-ready products <ChevronDown size={16} /></Link></div><span className="catalogue-interrupt-index">Selected</span></aside>}</Fragment>)}</div>{filtered.length > editResults.length && <div className="catalogue-limit"><span className="eyebrow">More to explore</span><p>Continue browsing by department, SKU, pack format or product name to narrow the trade edit.</p></div>}</> : <div className="empty-state"><Search size={30} /><h2>No matching products found.</h2><p>Try another product term, SKU or department.</p><button type="button" className="button-secondary" onClick={reset}>Reset the catalogue</button></div>}
      </section>
    </div>
  </StoreLayout>;
}
