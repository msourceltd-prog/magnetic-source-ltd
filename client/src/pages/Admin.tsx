...
function AdminDashboard({ onSignOut }: { onSignOut: () => void }) {
  ...
  return <StoreLayout>
    ...
    <section className="trade-shell admin-shell section-space">
      <nav className="admin-tabs"><button className={tab === "products" ? "active" : ""} onClick={() => setTab("products")}><PackagePlus size={17} /> Products <b>{counts.products}</b></button><button className={tab === "categories" ? "active" : ""} onClick={() => setTab("categories")}><ImagePlus size={17} /> Categories <b>{counts.categories}</b></button><button className={tab === "orders" ? "active" : ""} onClick={() => setTab("orders")}><ShieldCheck size={17} /> Requests <b>{counts.orders}</b></button></nav>
