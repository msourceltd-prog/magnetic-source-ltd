/**
 * Trade Ledger, Recut: a no-payment order-review flow that records selected
 * basket lines without implying payment collection or fulfilment acceptance.
 */
...
export default function Checkout() {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!result.persisted) { setSubmitError("We could not save your order request. Please try again later or contact the trade desk."); setSubmitted(false); return; }
      clearCart();
      navigate(`/order-confirmation?ref=${reference}`);
    } catch { setSubmitError("We could not save your order request. Please try again later or contact the trade desk."); setSubmitted(false); }
  };
